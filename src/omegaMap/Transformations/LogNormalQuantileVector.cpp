...
const string LogNormalQuantileVectorTransformParameterNames[3] = {"mean","sd","quantile"};

LogNormalQuantileVectorTransform::LogNormalQuantileVectorTransform(const int n, string name, DAG* dag) : DAGcomponent(name,dag,"LogNormalQuantileVectorTransform"), Transformation(LogNormalQuantileVectorTransformParameterNames,3), _n(n), _mean_changed(true), _sd_changed(true), _quantile_changed(true), _x(n), _x_prev(n), _has_changed(n,true), _bad(n), _bad_prev(n) {
}

LogNormalQuantileVectorTransform::LogNormalQuantileVectorTransform(const LogNormalQuantileVectorTransform& x) : DAGcomponent(x), Transformation(x), _n(x._n), _mean_changed(x._mean_changed), _sd_changed(x._sd_changed), _quantile_changed(x._quantile_changed), _x(x._x), _x_prev(x._x_prev), _has_changed(x._has_changed), _bad(x._bad), _bad_prev(x._bad_prev)  {
}

int LogNormalQuantileVectorTransform::length() const {
	return _n;
}

double LogNormalQuantileVectorTransform::get_double(const int i) const {
...
void LogNormalQuantileVectorTransform::recalculate() const {
	...
	if(!sd>0) _bad = vector<bool>(_n,true);
		...
		for(pos=0;pos<_n;pos++) {
			...
			if(!(quantile>0 & quantile<1)) _bad[pos] = true;
				...
				_x[pos] = exp(mean+sd*standard_normal_quantile_function(quantile));
				_bad[pos] = false;
			}
		}
	}
...
vector<double> LogNormalQuantileVectorTransform::get_doubles() const {
...
bool LogNormalQuantileVectorTransform::has_changed(const int i) const {
	return _has_changed[i];
}

vector<bool> LogNormalQuantileVectorTransform::has_changed() const {
	return _has_changed;
}

bool LogNormalQuantileVectorTransform::check_parameter_type(const int i, Variable* parameter) {
	switch(i) {
		case 0:	// mean
		...
		default:
			error("LogNormalQuantileVectorTransform::check_parameter_type(): parameter not found");
	}
	return false;
}

void LogNormalQuantileVectorTransform::set_mean(ContinuousVariable* mean) {
	set_parameter(0,(Variable*)mean);
}

void LogNormalQuantileVectorTransform::set_sd(ContinuousVariable* sd) {
	set_parameter(1,(Variable*)sd);
}

void LogNormalQuantileVectorTransform::set_quantile(ContinuousVectorVariable* quantile) {
	set_parameter(2,(Variable*)quantile);
}

ContinuousVariable const* LogNormalQuantileVectorTransform::get_mean() const {
	return (ContinuousVariable const*)get_parameter(0);
}

ContinuousVariable const* LogNormalQuantileVectorTransform::get_sd() const {
	return (ContinuousVariable const*)get_parameter(1);
}

ContinuousVectorVariable const* LogNormalQuantileVectorTransform::get_quantile() const {
	return (ContinuousVectorVariable const*)get_parameter(2);
}

double LogNormalQuantileVectorTransform::standard_normal_quantile_function(const double q) const {
	return gsl_cdf_ugaussian_Pinv(q);
}

void LogNormalQuantileVectorTransform::receive_signal_from_parent(const Value* v, const Variable::Signal sgl) {
