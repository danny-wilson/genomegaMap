 ...
 *
 */
#ifndef _MKPRF3_HMM_MAG_PATH_SAMPLER_H_
#define _MKPRF3_HMM_MAG_PATH_SAMPLER_H_
...
class mkprf3HMMmagPathSampler : public ContinuousVectorVariable, public Transformation {
private:
	string _distribution_name;		// Initially store just the name, until validate() is called
	mutable mkprf3HMMmag* _hmm;		// Distribution
...
public:
	// Constructor
	mkprf3HMMmagPathSampler(const int species, string rv_name, string distribution_name, string name="", DAG* dag=0);
	// Copy constructor
	mkprf3HMMmagPathSampler(const mkprf3HMMmagPathSampler& x);
...
#endif // _MKPRF3_HMM_MAG_PATH_SAMPLER_H_
