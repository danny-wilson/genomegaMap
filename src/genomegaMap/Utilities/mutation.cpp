...
// Some compilers prefer simply 
//#include "paml.h"
#ifdef __NOEXTERN_FOR_CINCLUDE
#include <genomegaMap/Utilities/paml.h>
#else
extern "C" {
#include <genomegaMap/Utilities/paml.h>
}
...
Mutation_Matrix::Mutation_Matrix(const Mutation_Matrix &M) {
	n_states = M.n_states;
...
Mutation_Matrix& Mutation_Matrix::operator=(const Mutation_Matrix& M) {
...
void Mutation_Matrix::set_state_freq(vector<double> state_freq_in) {
...
void Mutation_Matrix::set_state_char(vector<string> state_char_in) {
...
void Mutation_Matrix::set_ran(Random *ran_in) {
	ran = ran_in;
}

double Mutation_Matrix::get_rate(const int state) {
	if(state<0||state>=n_states) error("Mutation_Matrix::get_rate(state): unknown state");
	return mutation_rate[state];
}

int Mutation_Matrix::draw() {
	int state;
	double rp;
	double U = ran->U();
...
int Mutation_Matrix::mutate(const int state) {
...
int Mutation_Matrix::mutate_edge(const int state, const double time) {
	int old_state = state;
...
int Mutation_Matrix::mutate_edge(const int state, const double time, int &nmut) {
	int old_state = state;
	if(old_state<0||old_state>=n_states) error("Mutation_Matrix::mutate_edge(state,time): unknown state");
	if(time<0.0) error("Mutation_Matrix::mutate_edge(state,time): time must be non-negative");
	double time_remaining = time;
...
double Mutation_Matrix::expected_rate() {
	double result = 0.0;
	int i;
	for(i=0;i<n_states;i++)
...
void Mutation_Matrix::build_Pt(Matrix<double>& Pt, const Matrix<double>& SymEigenvec, const Vector<double>& SymEigenval, const Vector<double>& pi, const double t) {
	static Vector<double> _E;
	const int n = SymEigenval.size();
	_E.resize(n);
	Pt.resize(n,n);
	int i,j,k;
	// For efficiency, precalcuate the diagonal
	for(i=0;i<n;i++) {
		_E[i] = exp(SymEigenval[i]*t);
	}
	...
	for(i=0;i<n;i++) {
		for(j=0;j<n;j++) {
			Pt[i][j] = 0.0;
			for(k=0;k<n;k++) {
				Pt[i][j] += SymEigenvec[i][k] * SymEigenvec[j][k] * _E[k];
			}
		}
	}
...
/*		scale	Mutation rate
 */
void Mutation_Matrix::build_Rt(Matrix<double>& Rt, const Matrix<double>& SymEigenvec, const Vector<double>& SymEigenval, const Vector<double>& pi, const double scale, const double lambda) {
	static Vector<double> _E;
	const int n = SymEigenval.size();
	_E.resize(n);
	Rt.resize(n,n);
	...
	// For efficiency, precalcuate the diagonal
	for(i=0;i<n;i++) {
		_E[i] = lambda/(lambda-2.0*scale*SymEigenval[i]);
	}
	// Do the main computation
	for(i=0;i<n;i++) {
		for(j=0;j<n;j++) {
			Rt[i][j] = 0.0;
			for(k=0;k<n;k++) {
				Rt[i][j] += SymEigenvec[i][k] * SymEigenvec[j][k] * _E[k];
			}
		}
	}
	// Introduce the equilibrium frequencies
...
void Nucleotide_Mutation_Matrix::set_defaults() {
	n_states = 4;
...
JC69::JC69(const double lambda, Random *ran_in) {
	set_defaults();
	ran = ran_in;
...
int JC69::fast_mutate(const int state, const double time) {
...
F81::F81(const double lambda, const vector<double> state_freq_in, Random *ran_in) {
	set_defaults();
	ran = ran_in;
...
K80::K80(const double lambda, const double kappa, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	C.resize(n_states,n_states);
	update(lambda,kappa);
...
K80& K80::update(const double lambda, const double kappa) {
	int i,j;
	for(i=0;i<4;i++)
		for(j=0;j<4;j++)
...
int K80::fast_mutate(const int state, const double time) {
...
HKY85::HKY85(const double lambda, const double kappa, const vector<double> state_freq_in, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	set_state_freq(state_freq_in);
	C.resize(n_states,n_states);
...
TN93::TN93(const double lambda, const double kappa_R, const double kappa_Y, const vector<double> state_freq_in, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	set_state_freq(state_freq_in);
	C.resize(n_states,n_states);
...
void Codon_Mutation_Matrix::set_defaults() {
	n_states = 64;
...
NY98::NY98(const double mu, const double kappa, const double omega, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	C.resize(n_states,n_states);
	D.resize(n_states,n_states);
...
NY98::NY98(const double mu, const double kappa, const double omega, const vector<double> &pi, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	set_state_freq(pi);
	C.resize(n_states,n_states);
...
Codon_Mutation_Matrix& NY98::update(const double mu, const double kappa, const double omega) {
	build_C(mu,kappa,omega,state_freq);
	initialize(n_states,&C);
	return *this;
}

Codon_Mutation_Matrix& NY98::update(const double mu, const double kappa, const double omega, const vector<double> &pi) {
	set_state_freq(pi);
	build_C(mu,kappa,omega,state_freq);
	initialize(n_states,&C);
	return *this;
...
Codon_Mutation_Matrix& NY98::build_C(const double mu, const double kappa, const double omega, const vector<double> &pi) {
	int i,j;
	/*Initialize to zero*/
...
NY98_61::NY98_61(const double mu, const double kappa, const double omega, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	C.resize(n_states,n_states);
	D.resize(n_states,n_states);
...
NY98_61::NY98_61(const double mu, const double kappa, const double omega, const vector<double> &pi, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	set_state_freq(pi);
	C.resize(n_states,n_states);
...
Codon_Mutation_Matrix& NY98_61::update(const double mu, const double kappa, const double omega) {
	build_C(mu,kappa,omega,state_freq);
	initialize(n_states,&C);
	return *this;
}

Codon_Mutation_Matrix& NY98_61::update(const double mu, const double kappa, const double omega, const vector<double> &pi) {
	set_state_freq(pi);
	build_C(mu,kappa,omega,state_freq);
	initialize(n_states,&C);
	return *this;
...
Codon_Mutation_Matrix& NY98_61::build_C(const double mu, const double kappa, const double omega, const vector<double> &pi) {
	NY98_61::build_C(C,mu,kappa,omega,pi);
	return *this;
}

void NY98_61::build_C(Matrix<double>& C, const double mu, const double kappa, const double omega, const vector<double> &pi) {
	C.resize(61,61);
	int i,j;
	/*Initialize to zero*/
...
void NY98_61::build_A(Matrix<double>& C, const double mu, const double kappa, const double omega, const vector<double> &pi) {
	C.resize(61,61);
	int i,j;
	/*Initialize to zero*/
	for(i=0;i<61;i++){for(j=0;j<61;j++)C[i][j]=0.0;}
...
void NY98_61::diagonalize_symmetric(Matrix<double>& Eigenvec, Vector<double>& Eigenval, const double kappa, const double omega, const vector<double>& pi) {
...
FSM_Binary::FSM_Binary(Random *ran_in) {
	set_defaults();
	ran = ran_in;
	C.resize(n_states,n_states);
	initialize(2,&C);
...
FSM_Binary::FSM_Binary(const double lambda, Random *ran_in) {
	set_defaults();
	ran = ran_in;
	C.resize(n_states,n_states);
	update(lambda);
...
FSM_Binary& FSM_Binary::update(const double lambda) {
...
int FSM_Binary::fast_mutate(const int state, const double time) {
