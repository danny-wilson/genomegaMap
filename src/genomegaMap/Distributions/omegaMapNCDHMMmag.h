 ...
 *
 */
#ifndef _OMEGAMAP_NCD_HMM_MAGNITUDE_H_
#define _OMEGAMAP_NCD_HMM_MAGNITUDE_H_
...
class omegaMapNCDHMMmag : public Distribution {
private:
	// Sequence length
	int _L;
	...
	bool _anc_changed, _theta_changed, _kappa_changed, _gamma_changed, _T_changed, _p_changed, _pi_changed;
...
public:
	// Constructor
	omegaMapNCDHMMmag(const int L, const int ng, string name="", DAG* dag=0);
	// Copy constructor
	//omegaMapNCDHMMmag(const omegaMapNCDHMMmag& x);
	...
	void set_qpos(ContinuousVariable* qpos);
	void set_qneu(ContinuousVariable* qneu);
	...
	const ContinuousVariable* get_qpos() const;
	const ContinuousVariable* get_qneu() const;
...
class omegaMapNCDHMMmagPathSampler : public ContinuousVectorVariable, public Transformation {
private:
	string _distribution_name;		// Initially store just the name, until validate() is called
	mutable omegaMapNCDHMMmag* _hmm;	// Distribution
	RandomVariable* _rv;			// RV index wrt the distribution
	mutable int _L;					// Length
	
public:
	// Constructor
	omegaMapNCDHMMmagPathSampler(string rv_name, string distribution_name, string name="", DAG* dag=0);
	// Copy constructor
	omegaMapNCDHMMmagPathSampler(const omegaMapNCDHMMmagPathSampler& x);
...
#endif // _OMEGAMAP_NCD_HMM_MAGNITUDE_H_
