...
const string mkprf3HMMParameterNames[15] = {"thetaS0","thetaR0","gamma0","tau0","p0",
											"thetaS1","thetaR1","gamma1","tau1","p1",
											"thetaS2","thetaR2","gamma2","tau2","p2"};

mkprf3HMM::mkprf3HMM(string name, DAG* dag) : DAGcomponent(name,dag,"mkprf3HMM"), Distribution(mkprf3HMMParameterNames,15),
_thetaS_changed(vector<bool>(3,true)), _thetaR_changed(vector<bool>(3,true)), _gamma_changed(vector<bool>(3,true)),
_tau_changed(vector<bool>(3,true)), _p_changed(vector<bool>(3,true)) {
}

mkprf3HMM::mkprf3HMM(const mkprf3HMM& x) : DAGcomponent(x), Distribution(x), 
_thetaS_changed(x._thetaS_changed), _thetaR_changed(x._thetaR_changed), _gamma_changed(x._gamma_changed),
_tau_changed(x._tau_changed), _p_changed(x._p_changed), _likpos(x._likpos), _previous_likpos(x._previous_likpos), 
_site(x._site), _bad(x._bad), _alpha(x._alpha), _previous_alpha(x._previous_alpha) {
}

bool mkprf3HMM::check_random_variable_type(RandomVariable* random_variable) {
...
bool mkprf3HMM::check_parameter_type(const int i, Variable* parameter) {
	switch(i) {
		case 0: case 5: case 10:	//	thetaS
			return(dynamic_cast<ContinuousVariable*>(parameter));
		case 1: case 6: case 11:	//	thetaR
			return(dynamic_cast<ContinuousVariable*>(parameter));
		case 2: case 7: case 12:	//	gamma
			return(dynamic_cast<ContinuousVectorVariable*>(parameter));
		case 3: case 8: case 13:	//	tau
			return(dynamic_cast<ContinuousVariable*>(parameter));
		case 4: case 9: case 14:	//	p
			return(dynamic_cast<ContinuousVariable*>(parameter));
		default:
			error("mkprf3HMM::check_parameter_type(): parameter not found");
	}
	return false;
}

void mkprf3HMM::set_thetaS(ContinuousVariable* thetaS, const int sp) {
	set_parameter(0+5*sp,(Variable*)thetaS);
}

void mkprf3HMM::set_thetaR(ContinuousVariable* thetaR, const int sp) {
	set_parameter(1+5*sp,(Variable*)thetaR);
}

void mkprf3HMM::set_gamma(ContinuousVectorVariable* gamma, const int sp) {
	set_parameter(2+5*sp,(Variable*)gamma);
}

void mkprf3HMM::set_tau(ContinuousVariable* tau, const int sp) {
	set_parameter(3+5*sp,(Variable*)tau);
}

void mkprf3HMM::set_p(ContinuousVariable* p, const int sp) {
	set_parameter(4+5*sp,(Variable*)p);
}

const ContinuousVariable* mkprf3HMM::get_thetaS(const int sp) const {
	return (const ContinuousVariable*)get_parameter(0+5*sp);
}

const ContinuousVariable* mkprf3HMM::get_thetaR(const int sp) const {
	return (const ContinuousVariable*)get_parameter(1+5*sp);
}

const ContinuousVectorVariable* mkprf3HMM::get_gamma(const int sp) const {
	return (const ContinuousVectorVariable*)get_parameter(2+5*sp);
}

const ContinuousVariable* mkprf3HMM::get_tau(const int sp) const {
	return (const ContinuousVariable*)get_parameter(3+5*sp);
}

const ContinuousVariable* mkprf3HMM::get_p(const int sp) const {
	return (const ContinuousVariable*)get_parameter(4+5*sp);
}

mydouble mkprf3HMM::likelihood(const RandomVariable* rv, const Value* val) {
	// Remember: there may be multiple child RVs!
	map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
	if(it==_rv_index.end()) error("mkprf3HMM::likelihood(): RandomVariable not recognised");
	...
	for(sp=0;sp<3;sp++) {
		...
		if(_thetaS_changed[sp] || _thetaR_changed[sp] || _gamma_changed[sp] || _tau_changed[sp] || _p_changed[sp]) {
			// Pre-calculations!!!!!
			...
			pemiss = Matrix<mydouble>(5,ng);
			for(g=0;g<ng;g++) {
				const double GAMMA = get_gamma(sp)->get_double(g);
...
void mkprf3HMM::add_random_variable(RandomVariable* random_variable) {
	if(n_random_variables()>0) {
		string errTxt = "mkprf3HMM::add_random_variable(): only one child RV allowed for object " + name();
		error(errTxt.c_str());
	}
...
void mkprf3HMM::remove_random_variable(RandomVariable* random_variable) {
	error("mkprf3HMM::remove_random_variable(): not permitted");
}

void mkprf3HMM::receive_signal_from_parent(const Value* v, const Variable::Signal sgl) {
...
void mkprf3HMM::codify_sites(Codon61GroupCount* rv, const int ix) {
...
double mkprf3HMM::hypergeometric1F1(const double a, const double b, const double c) const {
	gsl_sf_result res;
	gsl_sf_hyperg_1F1_e(a, b, c, &res);
	return res.val;
}

double mkprf3HMM::F(const double n, const double theta, const double gamma) const {
	double ret;
...
double mkprf3HMM::G(const double n, const double theta, const double gamma) const {
	double ret;
...
void mkprf3HMM::summarize(ostream& out, Codon61GroupCount* rv, const int ix) {
	int pos, nNA=0;
	vector<int> n1(3,0), n2(3,0), n3(3,0), n4(3,0);
	const int L = rv->length();
	out << "mkprf3HMM summary for rv " << rv->name() << endl;
	out << endl;
...
vector<double> mkprf3HMM::path_sampler(const RandomVariable* const rv, const int sp) {
	// Remember: there may be multiple child RVs!
	...
	map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
	if(it==_rv_index.end()) error("mkprf3HMM::path_sampler(): RandomVariable not recognised");
	const int ix = it->second;
	...
	{	// pos = L-1
		...
		if(g==ng) error("mkprf3HMM::path_sampler(): did not draw path correctly");
		draw[L-1] = g;
		ret[L-1] = gamma[g];
	}
