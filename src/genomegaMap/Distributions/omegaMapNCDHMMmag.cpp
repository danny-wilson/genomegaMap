 ...
 */
/*
 *  omegaMapNCDHMMmagmag.cpp
 *  gcat
 *
 *  Created by Daniel Wilson on 11/05/2010.
 *  Copyright 2010 __MyCompanyName__. All rights reserved.
 *
 */
#include <DAG/DAG.h>
#include <omegaMap/Distributions/omegaMapNCDHMMmag.h>
...
const string omegaMapNCDHMMmagParameterNames[9] = {"anc","theta","kappa","gamma","T","p","pi","qpos","qneu"};

omegaMapNCDHMMmag::omegaMapNCDHMMmag(const int L, const int ng, string name, DAG* dag) : DAGcomponent(name,dag,"omegaMapNCDHMMmag"), Distribution(omegaMapNCDHMMmagParameterNames,9), _L(L), _ng(ng), _anc_changed(true),
_theta_changed(true), _kappa_changed(true), _gamma_changed(true), _T_changed(true), _p_changed(true), _pi_changed(true), _update_phylo_rate_matrix(true), _update_popgen_rate_matrix(true), _update_pclik(true),
...
/*omegaMapNCDHMMmag::omegaMapNCDHMMmag(const omegaMapNCDHMMmag& x) : DAGcomponent(x), Distribution(x), _anc_changed(x._anc_changed),
 _theta_changed(x._theta_changed), _kappa_changed(x._kappa_changed), _gamma_changed(x._gamma_changed), _T_changed(x._T_changed), _p_changed(x._p_changed), _pi_changed(x._pi_changed),
...
bool omegaMapNCDHMMmag::check_random_variable_type(RandomVariable* random_variable) {
...
bool omegaMapNCDHMMmag::check_parameter_type(const int i, Variable* parameter) {
	switch(i) {
		case 0:	{	//	anc
			// Insist on the particular RandomVariable (not just Variable) as it also specifies the encoding
			...
			if(!(rv==0) && rv->length()!=seqlen()) {
				stringstream errMsg;
				errMsg << "omegaMapNCDHMMmag::check_parameter_type(): Codon61SequenceRV object " << rv->name() << " has sequence length " << rv->length();
				errMsg << " whereas omegaMapNCDHMMmag object " << name() << " has sequence length " << seqlen();
		...
		case 3:	{	//	gamma
			...
			if(!(rv==0) && rv->length()!=ngamma()) {
				stringstream errMsg;
				errMsg << "omegaMapNCDHMMmag::check_parameter_type(): ContinuousVectorVariable object " << rv->name() << " has " << rv->length();
				errMsg << " entries whereas omegaMapNCDHMMmag object " << name() << " requires " << ngamma();
				error(errMsg.str().c_str());
			}
			return rv;
		}
		...
		case 8:		//	qneu
			return(dynamic_cast<ContinuousVariable*>(parameter));
		default:
			error("omegaMapNCDHMMmag::check_parameter_type(): parameter not found");
	}
	return false;
}

int omegaMapNCDHMMmag::seqlen() const {
	return _L;
}

int omegaMapNCDHMMmag::ngamma() const {
	return _ng;
}

void omegaMapNCDHMMmag::set_anc(Codon61SequenceRV* anc) {
	set_parameter(0,(Variable*)anc);
}

void omegaMapNCDHMMmag::set_theta(ContinuousVariable* theta) {
	set_parameter(1,(Variable*)theta);
}

void omegaMapNCDHMMmag::set_kappa(ContinuousVariable* kappa) {
	set_parameter(2,(Variable*)kappa);
}

void omegaMapNCDHMMmag::set_gamma(ContinuousVectorVariable* gamma) {
	set_parameter(3,(Variable*)gamma);
}

void omegaMapNCDHMMmag::set_T(ContinuousVariable* T) {
	set_parameter(4,(Variable*)T);
}

void omegaMapNCDHMMmag::set_p(ContinuousVariable* p) {
	set_parameter(5,(Variable*)p);
}

void omegaMapNCDHMMmag::set_pi(ContinuousVectorVariable* pi) {
	set_parameter(6,(Variable*)pi);
}

void omegaMapNCDHMMmag::set_qpos(ContinuousVariable* qpos) {
	set_parameter(7,(Variable*)qpos);
}

void omegaMapNCDHMMmag::set_qneu(ContinuousVariable* qneu) {
	set_parameter(8,(Variable*)qneu);
}

const Codon61SequenceRV* omegaMapNCDHMMmag::get_anc() const {
	return (const Codon61SequenceRV*)get_parameter(0);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_theta() const {
	return (const ContinuousVariable*)get_parameter(1);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_kappa() const {
	return (const ContinuousVariable*)get_parameter(2);
}

const ContinuousVectorVariable* omegaMapNCDHMMmag::get_gamma() const {
	return (const ContinuousVectorVariable*)get_parameter(3);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_T() const {
	return (const ContinuousVariable*)get_parameter(4);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_p() const {
	return (const ContinuousVariable*)get_parameter(5);
}

const ContinuousVectorVariable* omegaMapNCDHMMmag::get_pi() const {
	return (const ContinuousVectorVariable*)get_parameter(6);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_qpos() const {
	return (const ContinuousVariable*)get_parameter(7);
}

const ContinuousVariable* omegaMapNCDHMMmag::get_qneu() const {
	return (const ContinuousVariable*)get_parameter(8);
}

mydouble omegaMapNCDHMMmag::likelihood(const RandomVariable* rv, const Value* val) {
	// Remember: there may be multiple child RVs!
	map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
	if(it==_rv_index.end()) error("omegaMapNCDHMMmag::likelihood(): RandomVariable not recognised");
...
void omegaMapNCDHMMmag::add_random_variable(RandomVariable* random_variable) {
	...
	if(rv->length()!=seqlen()) {
		stringstream errMsg;
		errMsg << "omegaMapNCDHMMmag::add_random_variable(): Codon61Count object " << rv->name() << " has sequence length " << rv->length();
		errMsg << " whereas omegaMapNCDHMMmag object " << name() << " has sequence length " << seqlen();
		error(errMsg.str().c_str());
	}
	// Memory allocation
...
void omegaMapNCDHMMmag::remove_random_variable(RandomVariable* random_variable) {
	error("omegaMapNCDHMMmag::remove_random_variable(): not permitted");
}

void omegaMapNCDHMMmag::receive_signal_from_parent(const Value* v, const Variable::Signal sgl) {
	if(sgl==Variable::_ACCEPT || sgl==Variable::_REVERT) {
		_anc_changed = _theta_changed = _kappa_changed = _gamma_changed = _T_changed = _p_changed = _pi_changed = false;
	...
	if(_anc_changed || _theta_changed || _kappa_changed || _gamma_changed || _T_changed || _p_changed || _pi_changed) _update_alphabeta = true;
	Distribution::receive_signal_from_parent(v,sgl);
}

void omegaMapNCDHMMmag::update_phylo_rate_matrix() {
...
void omegaMapNCDHMMmag::update_popgen_rate_matrix() {
...
void omegaMapNCDHMMmag::update_pclik(const Value* val, const int ix) {
...
void omegaMapNCDHMMmag::update_pemiss(const int ix) {
...
void omegaMapNCDHMMmag::update_alphabeta(const int ix) {
	...
	bool full_recalc = (_theta_changed || _kappa_changed || _gamma_changed || _T_changed || _pi_changed || _p_changed);
	#ifdef _DEBUG_OMEGAMAP_NCD_HMM
	full_recalc = true;
	#endif
...
int omegaMapNCDHMMmag::recentre() const {
	return (_L-1)/2;
}

void omegaMapNCDHMMmag::forward(const int ix, const int left) {
	...
	for(;pos<=_likpos[ix];pos++) {
		...
		for(g=0;g<_ng;g++) {
			alpha[pos][g] = (sumalpha+pnoswitch*alpha[pos-1][g])*pemiss[pos][g];
			newsumalpha += alpha[pos][g];
		}
		newsumalpha *= pswitchpg;
		sumalpha = newsumalpha;
	}
}

void omegaMapNCDHMMmag::backward(const int ix, const int right) {
	...
	if(pos==_L-1) {
		...
		for(g=0;g<_ng;g++) {
			sumbeta += pemiss[pos+1][g]*beta[pos+2][g];
		}
		sumbeta *= pswitchpg;
	}
	for(;pos>=_likpos[ix];pos--) {
		...
		for(g=0;g<_ng;g++) {
			...
			newsumbeta += pemiss[pos][g]*beta[pos+1][g];
		}
		newsumbeta *= pswitchpg;
		sumbeta = newsumbeta;
	}
}

mydouble omegaMapNCDHMMmag::HMMlik(const int ix) {
	mydouble lik = 0.0;
	int g;
	for(g=0;g<_ng;g++) {
...
vector<double> omegaMapNCDHMMmag::path_sampler(const RandomVariable* const rv) {
	...
	map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
	if(it==_rv_index.end()) error("omegaMapNCDHMMmag::path_sampler(): RandomVariable not recognised");
	const int ix = it->second;
	...
	if(g==_ng) error("omegaMapNCDHMMmag::path_sampler(): did not draw path correctly");
	draw[pos] = g;
	ret[pos] = gamma[g];
	// Work leftwards
	for(pos=_likpos[ix]-1;pos>=0;pos--) {
		denom = 0;
		for(g=0;g<_ng;g++) {
			pdraw[g] = alphabeta[pos][g]*pswitchpg;
	...
	for(pos=_likpos[ix]+1;pos<_L;pos++) {
		denom = 0;
		for(g=0;g<_ng;g++) {
			pdraw[g] = alphabeta[pos+1][g]*pswitchpg;
...
const string omegaMapNCDHMMmagPathSamplerParameterNames[0];

omegaMapNCDHMMmagPathSampler::omegaMapNCDHMMmagPathSampler(string rv_name, string distribution_name, string name, DAG* dag) : DAGcomponent(name,dag,"omegaMapNCDHMMmagPathSampler"), Transformation(omegaMapNCDHMMmagPathSamplerParameterNames,0), _distribution_name(distribution_name) {
	_rv = getDAG()->get_random_variable(rv_name);
}

omegaMapNCDHMMmagPathSampler::omegaMapNCDHMMmagPathSampler(const omegaMapNCDHMMmagPathSampler& x) : DAGcomponent(x), Transformation(x), _distribution_name(x._distribution_name), _hmm(x._hmm), _rv(x._rv), _L(x._L) {
}

string omegaMapNCDHMMmagPathSampler::validate() const {
	// For starters, do not allow this to parameterize anything else
	if(n_child_distributions()+n_child_transformations()>0) {
		string errTxt = "omegaMapNCDHMMmagPathSampler: object " + name() + " may not parameterize other objects";
		return errTxt;
	}
	...
	_hmm = dynamic_cast<omegaMapNCDHMMmag*>(getDAG()->get_distribution(_distribution_name));
	if(!_hmm) {
		string errTxt = "omegaMapNCDHMMmagPathSampler: object " + name() + " cannot find omegaMapNCDHMMmag object " + _distribution_name;
		return errTxt;
	}
	const Codon61Count* ct = dynamic_cast<const Codon61Count*>(_rv);
	if(!ct) {
		string errTxt = "omegaMapNCDHMMmagPathSampler: " + _rv->name() + " is not of type Codon61Count";
		return errTxt;
	}
	_L = ct->length();
	return "";
...
int omegaMapNCDHMMmagPathSampler::length() const {
	return _L;
}
...
double omegaMapNCDHMMmagPathSampler::get_double(const int i) const {
	if(i<0) error("omegaMapNCDHMMmagPathSampler::get_double(): index cannot be negative");
	vector<double> x = get_doubles();
	if(i>=_L) error("omegaMapNCDHMMmagPathSampler::get_double(): index too large");
	return x[i];
}

vector<double> omegaMapNCDHMMmagPathSampler::get_doubles() const {
	return _hmm->path_sampler(_rv);
}

bool omegaMapNCDHMMmagPathSampler::has_changed(const int i) const {
	return true;
}

vector<bool> omegaMapNCDHMMmagPathSampler::has_changed() const {
	return vector<bool>(length(),true);
}

bool omegaMapNCDHMMmagPathSampler::check_parameter_type(const int i, Variable* parameter) {
	switch(i) {
		default:
			error("omegaMapNCDHMMmagPathSampler::check_parameter_type(): parameter not found");
	}
	return false;
}

void omegaMapNCDHMMmagPathSampler::print(ostream& out, string sep) {
	int i;
	vector<double> x = get_doubles();
	for(i=0;i<length();i++) {
		if(i>0) out << sep;
